...
function Forgetpass() {
  ...
  const handleemail = () => {
    axios
      .post("http://localhost:4000/user/forget", email)
      .then((res) => {
  ...
  const handleotp = () => {
    axios
      .post("http://localhost:4000/user/verify", otpvalue)
      .then((res) => {
        toast.success(res.data.message);
  ...
  return (
    <div
      class={`${
        show ? "flex" : "hidden"
      } h-screen w-full items-center justify-center fixed top-0 left-0 z-50 bg-gray-500 bg-opacity-75 bg-cover bg-no-repeat`}
    >
        ...
        <div className="text-white">
          <div className="mb-8 flex flex-col items-center">
            <h1 className="mb-2 text-2xl">
              <FontAwesomeIcon icon={faBagShopping} /> FUSION WHEELS
            </h1>
            <span className="text-gray-300">Welcome To FUSION WHEELS</span>
          </div>
          <form>
            {showotp ? (
