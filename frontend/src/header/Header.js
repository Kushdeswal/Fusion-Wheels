...
import { faCartShopping,  faList, faEllipsis, faCircleXmark, faMagnifyingGlass ,faCarOn,faGears ,faHouse} from "@fortawesome/free-solid-svg-icons";
    ...
    return (
        <>
            <nav id="header" className="w-full z-30  py-1 sticky top-0 bg-white">
                <div className="w-full container mx-auto flex flex-wrap items-center justify-between mt-0 px-6 py-3 ">
                    ...
                    <div className={`${menu ? "hidden" : "block"} shadow-2xl shadow-slate-400 md:shadow-none md:flex md:items-center md:w-auto w-full order-3 md:order-1`} id="menu">
                        <nav>
                            <ul className="md:flex items-center justify-between text-base text-gray-700 pt-4 md:pt-0">
                                <li><Link to="/" className="inline-block relative no-underline  py-2 px-4 changehover" ><FontAwesomeIcon icon={faHouse} className="file: mt-2 h-6 w-6" /></Link></li>

                                <li onClick={handlesetting}><Link className={`inline-block no-underline  relative py-2 px-4 changehover `} ><FontAwesomeIcon icon={faGears}className="file: mt-2 h-6 w-6" /></Link></li>

                                <li  onClick={()=>setsearch(!search)}><Link className={`${location.pathname === "/search" ? "hidden" : ""} inline-block no-underline  relative py-2 px-4 changehover`} ><FontAwesomeIcon icon={faMagnifyingGlass} className="file: mt-2 h-5 w-6" /></Link></li>
                    ...
                    <div className="order-1 md:order-2">
                        <a className="flex items-center tracking-wide no-underline hover:no-underline font-bold text-black text-xl " >
                           
                            FUSION  <FontAwesomeIcon icon={faCarOn}className="p-1" /> WHEELS
                        </a>
                    </div>
            ...
            <div className={`w-60 ${profile ? "hidden" : "block"} bg-white border border-gray-200 rounded-lg shadow absolute right-10 z-10`}>
                ...
                <div className="flex flex-col px-2 z-50 items-center pb-10">
            ...
            {/* search bar */}
            <div className={`container px-2 sm:px-10 ${location.pathname === "/search" ||  search  ? "":  "hidden"} absolute z-20  `}>
